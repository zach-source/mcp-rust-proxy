...
use mcp_proxy_core::config::ServerConfig;
use mcp_proxy_core::transport::{create_transport, Transport};
use mcp_proxy_core::{error::ServerError, Result};
use std::sync::Arc;
...
impl ManagedServer {
    pub async fn new(name: String, config: ServerConfig, state: Arc<AppState>) -> Result<Self> {
    ...
    pub async fn start(&self) -> Result<()> {
        ...
        // Update state
        self.state
            .set_server_state(&self.name, ServerState::Starting)
            .await?;
        ...
        match self
            .state
            .connection_pool
            .add_server(self.name.clone(), self.transport.clone())
            .await
        {
            Ok(_) => {
                tracing::info!("Server {} started successfully", self.name);
                self.state
                    .set_server_state(&self.name, ServerState::Running)
                    .await?;
                Ok(())
            }
            Err(e) => {
                ...
                self.state
                    .set_server_state(&self.name, ServerState::Failed)
                    .await?;
                Err(e)
            }
        }
    ...
    pub async fn stop(&self) -> Result<()> {
        ...
        // Update state
        self.state
            .set_server_state(&self.name, ServerState::Stopping)
            .await?;
        ...
        // Update state
        self.state
            .set_server_state(&self.name, ServerState::Stopped)
            .await?;
    ...
    pub async fn handle_failure(&self, restart_count: u32) -> Result<()> {
        ...
        let delay =
            Duration::from_millis(self.config.restart_delay_ms * (restart_count + 1) as u64);
        tracing::info!("Waiting {:?} before restarting server {}", delay, self.name);
        sleep(delay).await;
