...
impl ServerManager {
    ...
    async fn start_all_servers(&self) -> Result<()> {
        ...
        let disabled_servers = self.state.disabled_servers.read().await;

        for (name, server_config) in &config.servers {
            // Skip disabled servers
            if disabled_servers.is_disabled(name) {
                tracing::info!("Skipping disabled server: {}", name);
                continue;
            }
    ...
    pub async fn start_server(&self, name: &str) -> Result<()> {
        ...
        let disabled_servers = self.state.disabled_servers.read().await;
        if disabled_servers.is_disabled(name) {
            return Err(mcp_proxy_core::ProxyError::InvalidRequest(format!(
                "Server {} is disabled",
                name
            )));
        }
