...
mod tests {
    use super::super::*;
    use crate::protocol::{JsonRpcMessage, JsonRpcV2Message, JsonRpcRequest, JsonRpcResponse, JsonRpcId};
    use serde_json::json;
    ...
    fn test_request_routing() {
        ...
        // Test tool routing
        assert_eq!(router.get_server_for_tool("tools/list"), Some("server1".to_string()));
        assert_eq!(router.get_server_for_tool("tools/other"), None);
        
        // Test resource routing
        assert_eq!(router.get_server_for_resource("resources/list"), Some("server2".to_string()));
