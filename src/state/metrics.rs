...
impl Metrics {
    pub fn new() -> Self {
        ...
        let total_servers = IntGauge::new("mcp_proxy_total_servers", "Total number of configured servers")
            .expect("metric creation failed");
        let running_servers = IntGauge::new("mcp_proxy_running_servers", "Number of currently running servers")
            .expect("metric creation failed");
        let failed_servers = IntGauge::new("mcp_proxy_failed_servers", "Number of failed servers")
            .expect("metric creation failed");
        
        let total_requests = IntCounter::new("mcp_proxy_requests_total", "Total number of proxy requests")
            .expect("metric creation failed");
        let failed_requests = IntCounter::new("mcp_proxy_requests_failed", "Total number of failed proxy requests")
            .expect("metric creation failed");
        ...
        let active_connections = IntGauge::new("mcp_proxy_active_connections", "Number of active connections")
            .expect("metric creation failed");
        let connection_errors = IntCounter::new("mcp_proxy_connection_errors_total", "Total number of connection errors")
            .expect("metric creation failed");
        
        let health_checks_total = IntCounter::new("mcp_proxy_health_checks_total", "Total number of health checks")
            .expect("metric creation failed");
        let health_checks_failed = IntCounter::new("mcp_proxy_health_checks_failed", "Total number of failed health checks")
            .expect("metric creation failed");
        
        // Register all metrics
        registry.register(Box::new(total_servers.clone())).unwrap();
        registry.register(Box::new(running_servers.clone())).unwrap();
        ...
        registry.register(Box::new(health_checks_failed.clone())).unwrap();
